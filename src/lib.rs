...
impl Stream {
    pub fn new(id: StreamId, bytes: impl Into<Arc<[u8]>>, media_type: Option<String>) -> Self {
        Self { id, media_type, bytes: bytes.into() }
    }

    pub const fn id(&self) -> StreamId { self.id }
...
mod tests {
    ...
    fn stream_is_immutable_and_shareable() {
        let stream = Stream::new(StreamId::new(1), vec![1, 2, 3], Some("application/octet-stream".into()));
